...
function parseNoteMarker(content: string) {
  ...
  try {
    return JSON.parse(match[1]) as { title: string };
  } catch {
    return null;
  }
}
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  h1 { font-size: 2.5em; margin-bottom: 0.5em; color: #1e40af; }
  h2 { font-size: 1.8em; margin-bottom: 0.5em; color: #3b82f6; }
  p { font-size: 1.2em; line-height: 1.6; margin: 0.5em 0; }
  ul, ol { font-size: 1.1em; padding-left: 1.5em; margin: 0.5em 0; }
  li { margin: 0.4em 0; line-height: 1.5; }
  blockquote { 
    border-left: 4px solid #3b82f6; padding-left: 1em; 
    margin: 1em 0; font-style: italic; color: #64748b; 
  }
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center">
              ...
              <p className="text-[13px] text-muted-foreground mb-6 text-center max-w-md">
                I can create notes, generate presentations, organize your files,
                and much more. Just type or pick a prompt below.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-lg">
                {QUICK_PROMPTS.map((p) => (
                  <button
                    key={p}
                    onClick={() => send(p)}
                    className="text-left text-[12px] px-3 py-2.5 rounded-lg border border-border bg-background hover:bg-accent transition-colors text-foreground"
                  >
                    {p}
                  </button>
                ))}
            ...
            <AnimatePresence initial={false}>
              {messages.map((msg, i) => {
                ...
                const presentationMarker = !isUser
                  ? parsePresentationMarker(msg.content)
                  : null;
                  ...
                  >
                    ...
                    >
                      {isUser ? (
                        ...
                        <>
                          <div
                            className="text-[13px] leading-relaxed prose-editor"
