...
export default function GroupsPage() {
  ...
  const { data: groups = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("groups").select("*").in("id", memberships.map((m: any) => m.group_id)).order("updated_at", { ascending: false });
  ...
  const joinMutation = useMutation({
    mutationFn: async (code: string) => {
      const { data: group, error: gErr } = await supabase.from("groups").select("id, name").eq("invite_code", code.trim()).single();
      ...
      if (error) { if (error.code === "23505") throw new Error("You're already in this group"); throw error; }
      return group;
    },
    onSuccess: (group) => { queryClient.invalidateQueries({ queryKey: ["groups"] }); setJoinOpen(false); setInviteCode(""); setActiveGroup(group.id); toast.success(`Joined "${group.name}"!`); },
  ...
  if (activeGroup) return <GroupDetail groupId={activeGroup} onBack={() => setActiveGroup(null)} />;
    ...
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 max-w-md">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search groups…" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-9 rounded-xl bg-secondary/50 border border-border text-sm" />
          </div>
          <Button variant="outline" onClick={() => setJoinOpen(true)} size="sm" className="h-9 rounded-xl gap-1.5">
            <LogIn className="h-3.5 w-3.5" /> Join
          </Button>
          <Button onClick={() => setCreateOpen(true)} size="sm" className="h-9 rounded-xl gap-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <AnimatePresence>
            {filtered.map((group: any) => (
              <motion.div key={group.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                className="glass-card p-4 cursor-pointer hover:border-foreground/20 transition-colors group"
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader><DialogTitle>Create Group</DialogTitle><DialogDescription>Create a new collaborative workspace</DialogDescription></DialogHeader>
          <div className="space-y-3">
            <Input placeholder="Group name" value={newName} onChange={(e) => setNewName(e.target.value)} className="rounded-xl" />
      ...
      <Dialog open={joinOpen} onOpenChange={setJoinOpen}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader><DialogTitle>Join Group</DialogTitle><DialogDescription>Enter the invite code</DialogDescription></DialogHeader>
          <Input placeholder="Paste invite code here" value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} onKeyDown={(e) => e.key === "Enter" && inviteCode.trim() && joinMutation.mutate(inviteCode)} className="rounded-xl" />
          <DialogFooter>
            <Button onClick={() => joinMutation.mutate(inviteCode)} disabled={!inviteCode.trim() || joinMutation.isPending} className="rounded-xl">
              {joinMutation.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} Join
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
